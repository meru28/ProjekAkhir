...
class CartBertasbih extends Component {
  ...
  onBtnDelClick = id => {
    if (window.confirm("Yakin nih bro?")) {
      axios
        .delete("http://localhost:1997/order/" + id)
        .then(res => {
          this.renderlistCart();
        })
        .catch(err => {
  ...
  onCheckOutClick = () => {
    if (window.confirm("Yakin mau checkout nih bro?")) {
      axios
        .post("http://localhost:1997/history", {
          username: this.props.username,
        ...
        .then(res => {
          ...
          for (let i = 0; i < this.state.listCart.length; i++) {
            axios
              .delete(
                "http://localhost:1997/order/" + this.state.listCart[i].id
              )
              .then(res => {
                console.log(res);
                this.getCartList();
      ...
      return (
        <div className="container">
          ...
          <table style={{ marginTop: "40px" }}>
            ...
            <tbody>{this.renderBodyCart()}</tbody>
          </table>
          <center>{this.renderTotalHarga()}</center>
        </div>
      );
    } else {
      return (
